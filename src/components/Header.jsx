...
const Header = () => {
  ...
  return (
      <header className="bg-gradient-to-r from-green-100 via-green-50 to-emerald-100 shadow-lg border-b border-green-200">
        ...
        {isOpen && (
            <nav className="md:hidden py-4 bg-green-50">
              <ul className="flex flex-col items-center space-y-4">
                {navItems.map((item) => (
                    <li key={item.name} className="w-full">
                      <Link
                          to={item.path}
                          className="block py-2 px-4 text-center text-gray-700 hover:bg-green-500 hover:text-white rounded transition-colors"
